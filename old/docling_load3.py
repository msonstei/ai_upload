...
if __name__ == '__main__':
    ...
    file_id = file_id['id']
    response = check_file(file_id,OPENWEBUI_API_KEY)
    if response['status'] !='completed':
        ...
        if final.raise_for_status() != '200':
            print ("Error uploading knowedlge: {final.raise_for_status()}")
        else:
            print ("Successfully uploaded knowedlge")
