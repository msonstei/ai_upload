# vllm.py
# process large file directoried through ollama
import datetime
import logging
import time
...
def main():
    ...
    input_doc_path = data_folder / "pdf" /  "Contract.pdf"  # 18 pages

    vlm_options = ApiVlmOptions(
        #url="http://localhost:11434/api/embed",  # LM studio defaults to port 1234, VLLM to 8000
        url="http://du-webui/api/v1/chat/completions",
        params={
		"model": MODEL,
		"max_tokens": 4096,
		...
		#"temperature":0.0,
             ...
             # dict(
            #model=MODEL, #vlm_model_specs.GRANITEDOCLING_TRANSFORMERS.repo_id,
            #max_tokens=4096,
            #skip_special_tokens=True,
        #),
        prompt="Convert this page to markdown", #vlm_model_specs.GRANITEDOCLING_TRANSFORMERS.prompt,
        timeout=90,
        scale=2.0,
        temperature=0.0,
        concurrency=BATCH_SIZE,
        stop_strings=["", "<|end_of_text|>"],
        response_format=ResponseFormat.MARKDOWN, #DOCTAGS,
    )
